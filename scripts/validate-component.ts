...
const forbiddenPatterns: [RegExp, string][] = [
  ...
  [
    /from\s+['"]framer-motion['"]|from\s+['"]motion\/react['"]/,
    "Framer Motion imports are forbidden; use Motion for Vue",
  ],
...
function componentSlugs(files: string[]): string[] {
  const componentSourceFiles = files.filter((file) =>
    file.startsWith("docs/src/components/spark-ui/"),
  );
  if (componentSourceFiles.length === 0) return [];
...
function validateComponentArtifacts(slugs: string[]): string[] {
  ...
  for (const slug of slugs) {
    ...
    const componentFiles = listFiles(
      componentRoot,
      (file) => file.endsWith(".vue") && pathMatchesComponent(file, componentRoot, identifiers),
    );
    const docsFiles = listFiles(docsRoot, (file) =>
      pathMatchesComponent(file, docsRoot, identifiers),
    );
    const exampleFiles = exampleRoots.flatMap((root) =>
      listFiles(root, (file) => pathMatchesComponent(file, root, identifiers)),
    );
    const testFiles = listFiles(testsRoot, (file) =>
      pathMatchesComponent(file, testsRoot, identifiers),
    );
    ...
    if (exampleFiles.length === 0) failures.push(`${slug}: missing example or demo files`);
    if (testFiles.length === 0) failures.push(`${slug}: missing focused tests`);
  }
...
const failures = [...validateForbiddenSyntax(files), ...validateComponentArtifacts(slugs)];
