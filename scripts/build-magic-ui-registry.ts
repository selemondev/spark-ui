...
const ignoredNameFragments = [".test.", ".spec.", ".stories.", "index.", "utils.", "cn.", "use-"];
...
function metadataForDirectory(dir: string): MagicUiComponentMeta | null {
  ...
  const hasFramerMotion = /from\s+['"]framer-motion['"]|from\s+['"]motion\/react['"]|motion\./.test(
    sourceText,
  );
  const hasMotionUsage =
    hasFramerMotion ||
    /use(?:Scroll|Transform|Spring|MotionValue|InView)|AnimatePresence|layoutId|whileHover|whileTap|variants/.test(
      sourceText,
    );
  const hasAnimationUsage =
    hasMotionUsage ||
    /@keyframes|animate-|transition|duration-|ease-|spring|stagger/i.test(sourceText);
...
function relatedFiles(root: string, slug: string, extensions: Set<string>): string[] {
  ...
  return listFiles(root, (file) => {
    ...
    return (
      ...
      (basename === slug || basename.startsWith(`${slug}-`) || basename.includes(`-${slug}-`))
    );
  });
}
...
function absoluteManifestFile(manifestDir: string, filePath: string): string {
  ...
  if (existsSync(fromManifestDir)) return fromManifestDir;
...
function metadataForManifestItem(
  item: ManifestItem,
  manifestDir: string,
): MagicUiComponentMeta | null {
  ...
  const hasFramerMotion =
    dependencies.includes("framer-motion") ||
    /from\s+['"]framer-motion['"]|from\s+['"]motion\/react['"]/.test(sourceText);
  const hasMotionUsage =
    hasFramerMotion ||
    dependencies.includes("motion") ||
    /motion\.|use(?:Scroll|Transform|Spring|MotionValue|InView)|AnimatePresence|layoutId|whileHover|whileTap|variants/.test(
      sourceText,
    );
  const hasAnimationUsage =
    hasMotionUsage ||
...
if (manifestPath) {
  ...
  for (const item of manifest.items ?? []) {
    ...
    if (metadata) seen.set(metadata.slug, metadata);
  }
}
