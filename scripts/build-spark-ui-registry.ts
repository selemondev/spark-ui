...
function matchingFiles(root: string, slug: string): string[] {
  ...
  return listFiles(root, (file) => pathMatchesComponent(file, root, identifiers)).map((file) =>
    relativePath(file),
  );
}

const components = listDirs(componentRoot)
  .map((dir) => {
    const slug = toKebabCase(path.basename(dir));
    const sourceFiles = listFiles(dir, (file) => [".vue", ".ts"].includes(path.extname(file))).map(
