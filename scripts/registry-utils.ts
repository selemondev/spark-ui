...
export function aliasCandidates(value: string, aliases: Aliases = {}): string[] {
  ...
  for (const [key, values] of Object.entries(aliases)) {
    ...
    const matchesKey =
      normalizedKey === target || normalizeName(normalizedKey) === normalizedTarget;
    const matchesValue = normalizedValues.some(
      (item) => item === target || normalizeName(item) === normalizedTarget,
    );
...
export function pathMatchesComponent(
  filePath: string,
  basePath: string,
  identifiers: string[],
): boolean {
  const candidateKeys = new Set(
    identifiers
      .flatMap((identifier) => [toKebabCase(identifier), normalizeName(identifier)])
      .filter(Boolean),
  );
