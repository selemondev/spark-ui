...
for (const [legacyRelativePath, canonicalRelativePath] of legacyPaths) {
  ...
  if (!canonicalExists) {
    if (checkOnly)
      throw new Error(
        `Legacy file ${legacyRelativePath} must be renamed to ${canonicalRelativePath}`,
      );
  ...
  if (normalizedContent(legacyPath) !== normalizedContent(canonicalPath))
    throw new Error(
      `Conflicting duplicate files detected: ${legacyRelativePath} and ${canonicalRelativePath}`,
    );

  if (checkOnly)
    throw new Error(
      `Duplicate file ${legacyRelativePath} should be removed; canonical file is ${canonicalRelativePath}`,
    );
...
for (const obsoleteDirectory of [] as string[]) {
  ...
  if (checkOnly) throw new Error(`Obsolete duplicate directory remains: ${obsoleteDirectory}`);

  rmSync(obsoletePath, { recursive: true, force: true });
  report.duplicatesRemoved.push(obsoleteDirectory);
}

const namingRoots = ["docs/src", "docs/.vitepress/components"];
...
const referenceRoots = ["docs", "scripts", "registry", ".github", ".ai", "package.json"];
...
for (const root of referenceRoots) {
  ...
  const files =
    absoluteRoot.endsWith(".json") || absoluteRoot.endsWith(".md") || absoluteRoot.endsWith(".yml")
      ? [absoluteRoot]
      : listFiles(absoluteRoot, (file) =>
          /\.(?:vue|ts|js|mjs|mts|md|mdx|json|yml|yaml)$/.test(file),
        );
  ...
  for (const file of files) {
    ...
    for (const fragment of stalePathFragments) {
      if (content.includes(fragment))
        throw new Error(
          `Stale legacy path fragment found in ${path.relative(repoRoot, file)}: ${fragment}`,
        );
    }
    ...
    for (const fileName of staleFileNames) {
      if (content.includes(fileName))
        throw new Error(
          `Stale legacy filename found in ${path.relative(repoRoot, file)}: ${fileName}`,
        );
    }
  }
}
...
console.log("Renamed:");
console.log(
  report.renamed.length > 0 ? report.renamed.map((entry) => `* ${entry}`).join("\n") : "* None",
);
console.log("");
console.log("Duplicates Removed:");
console.log(
  report.duplicatesRemoved.length > 0
    ? report.duplicatesRemoved.map((entry) => `* ${entry}`).join("\n")
    : "* None",
);
console.log("");
