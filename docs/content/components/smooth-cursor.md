...
A pointer that follows movement with spring motion and turns toward its direction.

<demo src="../../src/example/smooth-cursor/demo.vue" srcCode="../../src/spark-ui-demos/smooth-cursor/demo.vue" />

## Installation

Install `motion-v` before you copy the component.

Copy the file below into `src/components/spark-ui/smooth-cursor/`. Utility imports use `@/lib/utils`.

::: code-group

```vue [smooth-cursor.vue]
...
```vue
<script setup lang="ts">
import SmoothCursor from "@/components/spark-ui/smooth-cursor/smooth-cursor.vue";
</script>

<template>
  <div class="h-80 rounded-xl border p-8">
...
The component handles at most one update per animation frame and uses the latest pointer position. Touch input never displays the effect or hides the native cursor. Fine pointer support enables mouse and pen tracking. Only mouse input hides the native cursor.

The component restores the previous cursor style on leave, scrolling, lost focus, hidden pages, and unmount. It cancels pending frames and timers and removes all listeners on unmount. Do not add global `cursor: none` CSS. That rule can hide the cursor when this component is inactive.
...
`damping` controls how quickly motion settles. `stiffness` controls the pull toward the target. `mass` controls resistance to movement. `restDelta` is the remaining distance at which motion can stop.

Rotation uses damping `60` and stiffness `300`. Scale uses damping `35` and stiffness `500`. Both use `mass` and `restDelta` from `springConfig`.

## Source

Ported from [Magic UI Smooth Cursor](https://magicui.design/docs/components/smooth-cursor). The [upstream source](https://github.com/magicuidesign/magicui/blob/main/apps/www/registry/magicui/smooth-cursor.tsx) uses the MIT license.
