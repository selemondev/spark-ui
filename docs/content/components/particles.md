...
<template>
  <svg viewBox="0 0 1440 181" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M0 0H1440V181H0V0Z" :fill="`url(#${backgroundGradientId})`" fill-opacity="0.22" />
    <path d="M0 2H1440V-2H0V2Z" :fill="`url(#${borderGradientId})`" :mask="`url(#${maskId})`" />
    <defs>
      <linearGradient
...
| ---------- | ------- | --------------------------------------------------- | --------- |
| color      | string  | The color of both the `particles` and `background`. | '#00DC82' |
| accelerate | boolean | If true, the animation plays quickly.               | false     |

Both components use the provided `color` directly and have no VitePress dependency. For light/dark adaptation, bind each component's `color` to your application's theme state. Background masks and gradients use per-instance identifiers, so differently colored backgrounds can coexist.
