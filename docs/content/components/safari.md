...
Show an image or video inside a Safari browser frame.

<demo src="../../src/example/safari/demo.vue" srcCode="../../src/spark-ui-demos/safari/demo.vue" />

## Installation

Copy the component below into `src/components/spark-ui/safari/safari.vue`.
...
const props = withDefaults(defineProps<SafariProps>(), { mode: "default", imageAlt: "" });
const id = useId();
const punchId = `safari-punch-${id}`;
const clipId = `safari-clip-${id}`;
const hasMedia = computed(() => Boolean(props.videoSrc || props.imageSrc));
const screenStyle = {
  left: `${100 / 1203}%`,
  top: `${5200 / 753}%`,
  width: `${120000 / 1203}%`,
  height: `${70000 / 753}%`,
  borderRadius: "0 0 0.916667% 0.916667% / 0 0 1.571429% 1.571429%",
};
</script>

<template>
  <div
    class="relative inline-block w-full align-middle leading-none"
    :class="className"
    style="aspect-ratio: 1203 / 753"
  >
    <div
      v-if="hasMedia"
    ...
    >
      <video
        ...
        preload="metadata"
      />
      <img v-else :src="imageSrc" :alt="imageAlt" class="block size-full object-cover object-top" />
    </div>
    <svg
    ...
    >
      ...
      <g :clip-path="`url(#${clipId})`" :mask="hasMedia ? `url(#${punchId})` : undefined">
        <path
...
```vue
<script setup lang="ts">
import Safari from "@/components/spark-ui/safari/safari.vue";
</script>
...
| ----------- | ----------------------- | ----------- | ------------------------------------- |
| `url`       | `string`                | Unset       | Text in the address bar.              |
| `imageSrc`  | `string`                | Unset       | Image URL.                            |
| `videoSrc`  | `string`                | Unset       | Video URL. Overrides the image.       |
| `mode`      | `"default" \| "simple"` | `"default"` | Controls extra toolbar icons.         |
| `className` | `string`                | Unset       | Extra wrapper classes.                |
| `imageAlt`  | `string`                | `""`        | Image description for screen readers. |

## Behavior

Set the wrapper width with `class` or `style`. The frame keeps its 1203:753 aspect ratio. Standard HTML attributes pass to the wrapper.

The screen opens only when media is present. Each instance uses separate SVG mask identifiers. Both media types clip to the rounded screen corners.

Video uses a normal HTML overlay for Safari and iOS support. It is muted, loops, plays inline, and requests automatic playback. Browser autoplay rules still apply.

The demo loads a mountain image from Unsplash. For video, supply a browser-supported video URL. No extra package is required.

## Source

Ported from [Magic UI Safari](https://magicui.design/docs/components/safari). The [upstream source](https://github.com/magicuidesign/magicui/blob/main/apps/www/registry/magicui/safari.tsx) uses React.
