...
A public tweet card with author details, linked text, photos, video, and link previews.

<demo src="../../src/example/tweet-card/demo.vue" srcCode="../../src/spark-ui-demos/tweet-card/demo.vue" />

## Installation

Copy all three files below into `src/components/spark-ui/tweet-card/`. The component uses Vue and your existing `@/lib/utils` helper.
No React package or extra CSS configuration is required.
...
Pass a public tweet ID to fetch its data. The default endpoint is the same public endpoint that React Tweet uses in the browser.

```vue
<script setup lang="ts">
import TweetCard from "@/components/spark-ui/tweet-card/tweet-card.vue";
</script>

<template>
  <TweetCard id="1668408059125702661" client-only>
...
Pass a `TweetData` object through `tweet` to render data that your application already loaded. Supplied data takes priority over `id` and `apiUrl`.
The demo uses a saved public response. Profile images and video still load from X media servers.
...
| `clientOnly`   | `boolean`                   | `false` | Skip the server request and load after mounting.                         |
...
The card links hashtags, mentions, symbols, and URLs from the tweet entities. An entity is a marked range in the tweet text.
Text keeps its line breaks and Unicode characters. Vue escapes all text, and links and media accept only HTTP or HTTPS URLs.
The card shows author details, verification, a UTC timestamp, counts, reply context, and one quoted tweet when those fields exist.

Photos scroll horizontally and support keyboard focus. Videos use native controls and do not autoplay.
The renderer selects an MP4 source rather than assuming that the first variant is playable. Sensitive media stays hidden until the reader selects its button.
Link previews appear when the response includes a thumbnail and no photo or video.

The loading skeleton respects reduced motion. A missing or private tweet shows the unavailable state.
A failed request shows the error state and calls `onError`. Missing tweets do not call `onError`.
Changes to `id`, `apiUrl`, `tweet`, or the `fetchOptions` object cancel the prior request. Unmounting cancels active requests and removes cancellation listeners.

## Server rendering and endpoints

Server rendering means that Vue builds the initial HTML on the server. `TweetCard` uses `onServerPrefetch` to load before that render.
For hydration, pass the same `tweet` data to the server and browser. Hydration connects the browser to the server HTML.
Frameworks such as Nuxt provide data-loading tools that transfer this state. Without shared data, use `ClientTweetCard` to keep both initial renders in the loading state.

A custom `apiUrl` must return JSON with the shape `{ data: TweetData | null }`. Pass the complete URL, including the ID.
The default endpoint is `https://react-tweet.vercel.app/api/tweet/{id}`. It is a third-party public service with no uptime guarantee.
Network restrictions, removed tweets, and rate limits can prevent a live request. For a production site, use your own server endpoint and cache public responses there.

The helper exports `getTweet(id, apiUrl?, fetchOptions?)` for that JSON endpoint.
It also exports `getTweetFromSyndication(id, fetchOptions?)` for server access to X's public syndication API.
Do not call the syndication helper from the browser. That endpoint does not provide a browser CORS contract.
CORS is the browser rule for access across origins. Both helpers return `null` for unavailable tweets and throw on request errors.

## Source

Ported from [Magic UI Tweet Card](https://github.com/magicuidesign/magicui/blob/main/apps/www/registry/magicui/tweet-card.tsx).
The fetch contract and entity ranges follow [React Tweet](https://github.com/vercel/react-tweet/tree/main/packages/react-tweet/src).
Slots replace React fallback nodes and the `components.TweetNotFound` override. Vue server prefetch replaces the React Server Component.
